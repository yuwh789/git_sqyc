...
def Test_files():
	...
	order_info_data_ordered['order_times'] =  order_info_data_ordered.groupby('user_id').agg({'order_time': 'cumsum'})['order_time']
	...
	merge_data = order_info_data_ordered.merge(right=order_info_data_ordered_2,
	    how='left', on=['user_id', 'order_times']).loc[:,['user_id', 'order_id', 'start_time', 'end_time', 'last_end_time']]
...
def Test_cany_day(t_date, t_cityId, t_canyName):
    psy = Psyco_handle()
    sql = "SELECT t1.*, t2.rard_punish_money  from fuc_company_day('2018-06-10', '94', '奔腾')  t1 \
    LEFT JOIN  fuc_driver_reward('2018-06-10','94','奔腾') t2 on t1.司机编码::varchar = t2.driv_id::varchar"
    df_company_day = psy.data_r(sql)
    df_company_day.fillna("0",inplace=True)
    df_company_day.rename(columns={"城市编码":"city_id","城市":"city","司机编码":"driver_id","司机姓名":"driver_name","司机电话":"driver_phone",
