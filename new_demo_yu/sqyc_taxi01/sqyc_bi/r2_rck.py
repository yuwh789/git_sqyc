...
def Risk_record(t_d, psy):
    ...
    where  create_time  >= '{}' and create_time < '{}'::date + INTERVAL '1 day' and warn_type ~ 'SMS|DRIVER_FOZEN' ".format(
        t_d, t_d)
...
def Df_drInfo(t_d, psy):
    ...
    null  and t1.pay_card_no is not null  and t1.create_time < '{}'::date + INTERVAL '1 day' \
    GROUP BY 1 ,2   HAVING COUNT(*)>2  ) t1 join  (	SELECT DISTINCT driver_id from mysql.risk_order t2 \
    where  create_time >=  '{}' and   create_time < '{}'::date + INTERVAL '1 day' ) t2 on  t2.driver_id = t1.driver_id  ".format(
        t_d, t_d, t_d)
    ...
    and   t1.create_time   <= '{}'::date + INTERVAL '1 day'	GROUP BY 1 ,2,3 HAVING COUNT(*)>2	) a1 JOIN  \
    ( SELECT  t2.driver_id  from mysql.risk_order  t2  where  create_time >='{}' and  create_time < '{}'::date + INTERVAL '1 day' ) a2  on a1.driver_id = a2.driver_id  GROUP BY 1 ,2".format(
        t_d, t_d, t_d)
...
def Fk_seven_data(t_d, psy):
    # 处理风控七日数据
    print('--- 风控七天订单数据 ---')
    sql = " select city_id ,order_no,driver_id,create_time ,remark,booking_user_phone,device_id,pay_card_no  from mysql.risk_order  \
    where create_time > '{}'::date - INTERVAL '7 day'  and  create_time < '{}'::date + INTERVAL '1 day'  and city_id is not null  ".format(
        t_d, t_d)
    ...
    res = pd.merge(res, d_phone, how='left', left_on=['driver_id', 'booking_user_phone'],
                   right_on=['driver_id', 'booking_user_phone'])

    #  支付账号握手频次处理
    d_payCard = df1.groupby(['driver_id', 'pay_card_no']).size().reset_index()
    d_payCard.rename(columns={0: 'd_payCard_num'}, inplace=True)
    res = pd.merge(res, d_payCard, how='left', left_on=['driver_id', 'pay_card_no'],
                   right_on=['driver_id', 'pay_card_no'])
...
def Passenger_info(t_d, psy):
    ...
    ( SELECT pay_card_no,driver_id,booking_user_id from  mysql.risk_order  where   create_time <  '{}'::date + INTERVAL '1 day' and pay_card_no is not null ) t1  join  \
    SELECT create_time, booking_user_id  from  mysql.risk_order   where   pay_card_no is not null  and (create_time  >='{}'  and create_time< '{}'::date + INTERVAL '1 day' )  )   t2   \
...
def Order_online(t_d, psy):
    ...
    on  t1.order_id = t2.order_id  join  (  SELECT order_id,  driver_id, driver_coordinate, MIN( grab_order_time ) grab_order_time   from  mysql.order_driver_event  \
    ...
    df_onDay['服务里程'] = df_onDay.apply(data_tools.Distance, axis=1)
    df_onDay['接驾里程'] = df_onDay.apply(data_tools.Distance2, axis=1)
    df_onDay2['fact_end_point'] = df_onDay2['上一单服务结束位置']
    df_onDay2['间隔里程'] = df_onDay2.apply(data_tools.Distance, axis=1)
    ...
    ret_xs = res_xs[
        ['order_id', 'city_id', 'service_end_date', 'driver_id', 'driver_name', 'booking_user_id', 'rider_user_phone',
         'fact_pay_amount',
    ...
    # 异常指标计算
    ret_xs['接驾异常'] = ret_xs.apply(data_tools.exc_j, args=(take_driver_num,), axis=1)
    ret_xs['服务异常'] = ret_xs.apply(data_tools.exc_fw, args=(service_num,), axis=1)
    ret_xs['两单间隔异常'] = ret_xs.apply(data_tools.exc_ldjg, args=(order_interval_num,), axis=1)
    ret_xs['多次握手异常'] = ret_xs.apply(data_tools.exc_dcws, axis=1)

    # 根据金额调整评分金额大于中位数：10倍+0.2， 2倍-0.4，大于中位数0.2，小于中位数则0 func_money
    ret_xs['金额异常'] = ret_xs.apply(data_tools.func_money, axis=1)
    ret_xs['订单异常'] = (ret_xs['接驾异常'] + ret_xs['服务异常'] + ret_xs['两单间隔异常'] + ret_xs['多次握手异常'] + ret_xs[
        '金额异常']) * args_value
...
def Online_order_handle(order_online, df_drInfo, passenger_info, df_lx):
    ...
    # 整合司机拉新
    order_online = pd.merge(order_online, df_lx[['driver_id', 'passenger_id', 'status']], how='left',
                            left_on=['driver_id', 'booking_user_id'], right_on=['driver_id', 'passenger_id'])
...
def Func_new(ol_handle):
    ...
    ol_handle['调整评分'] = ol_handle.apply(data_tools.ret_newScore, axis=1)
...
def Ret_table(ret_xs, df_fkDr):
    ...
    ret_fr_flx = ret_xs[ret_xs['status'] < 3].groupby(['driver_id', 'city_id']).size().reset_index().rename(
        columns={0: '非拉新订单'})
...
if __name__ == '__main__':
    ...
    psy.data_s(df_drInfo, 't_risk_driver_info')
    # df_drInfo.to_excel(writer,'司机信息表') # 司机信息表保存


    df_lx = Df_lx(t_d, psy)  # 司机拉新
    passenger_info = Passenger_info(t_d, psy)  # 乘客异常
    psy.data_s(passenger_info, 't_risk_passenger_info')
    # passenger_info.to_excel(writer,'乘客信息表')# 乘客信息表保存

    # 订单异常分析
    fk_s_d = Fk_seven_data(t_d, psy)  # 风控七日订单数据
    psy.data_s(fk_s_d, 't_risk_seven_order')
    # fk_s_d.to_excel(writer,'风控订单7天') # 风控订单7日表保存

    order_online = Order_online(t_d, psy2, fk_s_d)  # 线上订单数据
    ...
    psy.data_s(func_new, 't_risk_online_order')
    # func_new.to_excel(writer,'当日线上完单')# 当日线上完单保存

    ret_table = Ret_table(func_new, df_fkDr)  # 结果表生成
    psy.data_s(ret_table, 't_risk_result')
    # ret_table.to_excel(writer,'结果表')  # 结果表保存
    ...
    # data_tools.mail_mimemuprt("风控", path,to_address_list )


    print("")
    print("程序结束! 耗时: %.2f s" % (t2 - t1))
