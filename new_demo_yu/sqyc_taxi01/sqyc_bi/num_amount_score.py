...
def online_num_amount(psy, t_date, t_cityId):
    ...
    then t1.dis_count_amount+ t1.fact_pay_amount  else 0 end  ) ol_amount, count(*) com_cnt_order  from mysql.order_info t1  \
...
def Appra_score(psy, t_date):
    ...
    from mysql.order_appraisal  where create_date >= '{}' and create_date< '{}'::date + interval '1 day' GROUP BY 1 ".format(
        t_date, t_date)
    apr_sco = psy.data_r(sql)
    return apr_sco


def Run_num_amount_score(t_cityId):
    psy = Psyco_handle()
    date_list = Date_list()
    # time  handle
